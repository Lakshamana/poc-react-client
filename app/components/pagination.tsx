...
export function Pagination({ currentPage, hide, lastPage, onSelectedPage }: PaginationProps) {
  ...
  const pagesIterator = Array(lastPage)
    .fill(null)
  ...
  const firstPage = 1
  const alwaysShownPages = [firstPage, currentPage, lastPage]

  return (
    <div className='inline-flex rounded-sm mt-4'>
      <ul className='flex items-center'>
        {currentPage > firstPage && (
          <li className='px-2' onClick={e => handleSelectedPage(e, currentPage - 1)}>
            <button className='w-9 h-9 flex items-center justify-center rounded-md border disabled'>
              <span>
                <svg
                  width='8'
                  height='15'
        ...
        {currentPage < lastPage && (
          <li className='px-2' onClick={e => handleSelectedPage(e, currentPage + 1)}>
            <button className='w-9 h-9 flex items-center justify-center rounded-md border hover:text-indigo-500'>
              <span>
                <svg
                  width='8'
                  height='15'
